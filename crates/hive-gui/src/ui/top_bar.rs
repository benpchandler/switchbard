//! The two-row top panel: title + workspace-wide controls (filter, "show
//! non-server scripts" toggle, "only attributed" toggle, browser picker,
//! Refresh) + status badges.

use crate::app::HiveApp;
use crate::ui::unified;
use eframe::egui;
...
pub fn render(app: &mut HiveApp, ctx: &egui::Context) {
    egui::TopBottomPanel::top("top").show(ctx, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.label(
                egui::RichText::new("matches repo, branch, service, command, port, listener")
                    .weak(),
            );
            ui.separator();
            ui.checkbox(&mut app.show_only_managed, "only attributed");
...
fn render_actions(app: &mut HiveApp, ui: &mut egui::Ui) {
    ...
    ui.separator();
    let pgids = unified::unique_pgids_in_filter(app);
