...
- `group.rs` - `o`: headings + tasks over the filtered, sorted order; project headings: def status, done/total; goal headings: week actual/target, pace.
- `ball.rs` - who holds the ball: `ball:me`/`ball:agent` labels (`dispatching` = agent); `b` cycles. `a` / `:goal <name>`: attach or detach the task to a goal.
