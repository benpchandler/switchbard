...
- `app/` - `mod.rs` state, loop, browse keys, commands; `pickers.rs` column/filter/sort
  pickers + the shared picker key handler; `new_task.rs` the Tasks `t n` title draft and native create; `task_status.rs` / `task_project.rs` the `t s` status and `t p` project native edits with stable task IDs; `paint_flow.rs` the `p` flow; `slots.rs` the shared `v` open/save/global pickers.
