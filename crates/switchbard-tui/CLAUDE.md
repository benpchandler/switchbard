...
- Live work (core `work_sessions`, TASK-150): `work` column = one ● per live session (`sb work claim`), the row wears `working` for `work.flash_ms` of every `work.period_ms`, title counts `working:N`, detail names sessions; `w` passes the task (ends every claim).
