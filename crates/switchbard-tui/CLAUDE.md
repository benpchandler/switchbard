...
- `detail_pane.rs` / `app/detail.rs` - shared task/PR split, focus, pointer hit regions and bounded scrolling. Task Enter focuses details; Shift+Tab switches list/detail focus, Esc closes. Focused arrows/j/k, PageUp/Down, Ctrl-u/d and Home/End scroll; the wheel follows the pointer. PR Enter keeps list navigation until focus switches.
