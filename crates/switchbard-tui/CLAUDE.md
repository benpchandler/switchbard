...
- Live work (core `work_sessions`, TASK-150): `work` column = one ● per live session (`sb work claim`), the `working` band pulses (bright, fade out, fade in) every `work.period_ms`, `work.frames` redraws each, title counts `working:N`, detail names sessions; `w` passes the task (ends every claim).
