...
- `view_history.rs` and `view/history_picker.rs` - automatic per-repo history capture and the `v h` picker; Enter restores an arrangement and `v s <number>` deliberately promotes it to a slot. Capture is deduped globally with move-to-front and bounded by age, count, file size and entry size.
