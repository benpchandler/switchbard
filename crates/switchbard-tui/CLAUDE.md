...
- `paint.rs` - `p` rules in a hierarchy: `by:<col>=v:c,...`, `rows:<filter>=c`, `column:<col>=c`. Automatic palette choices persist as `p<n>` tokens and resolve against the current palette; explicit colors remain literal.
