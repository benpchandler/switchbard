...
fn detect_shell_scripts(root: &Path) -> Vec<DetectedService> {
    ...
    for subdir in ["scripts", "bin"] {
        ...
        for entry in entries.flatten() {
            ...
            let (likelihood, port_from_body) = match read_declaration_text(&path) {
...
fn detect_makefile(root: &Path) -> Vec<DetectedService> {
    ...
    let Ok(text) = read_declaration_text(&mk) else {
        return vec![];
    };
    let keywords: &[&str] = &[
...
fn detect_node_scripts(root: &Path) -> Vec<DetectedService> {
    ...
    let Ok(text) = read_declaration_text(&pj) else {
        return vec![];
    };
...
fn detect_procfile(root: &Path) -> Vec<DetectedService> {
    for name in ["Procfile", "Procfile.dev"] {
        ...
        let Ok(text) = read_declaration_text(&path) else {
            continue;
        };
        let mut out = Vec::new();
        for line in text.lines() {
...
fn read_declaration_text(path: &Path) -> std::io::Result<String> {
    let file = File::open(path)?;
    let mut limited = file.take(MAX_DECLARATION_BYTES as u64 + 1);
    let mut text = String::new();
    limited.read_to_string(&mut text)?;
    if text.len() > MAX_DECLARATION_BYTES {
        Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            "declaration file is too large",
        ))
    } else {
        Ok(text)
    }
...
mod tests {
    ...
    fn shell_script_port_pulled_from_body() {
        ...
        // The surface command Switchbard runs (`./scripts/start_lyon.sh`) reveals
...
"#;
    ...
    #[test]
    fn oversized_package_json_is_ignored() {
        let d = tmpdir();
        let padding = "a".repeat(1_048_576 + 1);
        fs::write(
            d.path().join("package.json"),
            format!(r#"{{"scripts":{{"dev":"vite --port 5173"}},"padding":"{padding}"}}"#),
        )
        .unwrap();

        assert!(detect_services(d.path()).is_empty());
    }
