...
Owner-directed 2026-09-08: replace default `a` with `t` then `n`. Help derives the chord from the configured task prefix, and the task menu leads with `n New`. Existing draft, error, cancel, filter, Unicode and PR coexistence E2Es now enter through the chord; an additional E2E checks removal of `a`, help, prefix cancellation and remapping. This is a key-routing change with the existing state matrix unchanged. Worktree feat/tui-task-new began clean at 48ec9f1. Isolated `mise run tui-install` passed: fmt, clippy, 103 tests, rustdoc and install. Existing 17 opt-in tests remain skipped.
