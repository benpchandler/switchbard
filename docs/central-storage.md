...
The preview inventories linked worktrees and relevant local branches. Apply rechecks its digest, validates native content, creates a private verified database backup and source manifest, and switches that kind transactionally. Native Switchbard legacy writers, migration, and import share a repository-identity lock, including linked worktrees; a writer takes it before authority selection whenever any kind it touches is still file-authoritative, and holds it through the complete write or activation, with authority rechecked after acquisition. Writes whose kinds are all central skip the lock: SQLite serializes them and each transaction rechecks authority, revisions, and sequences. This relies on authority only ever being granted, never revoked. Pause direct Markdown editors briefly during apply: arbitrary external writes cannot join this lock, so the retained digest and source checks remain the final guard. Original files stay unchanged. Git-proven stale committed copies can follow the primary version, with every original variant retained in provenance. Dirty, independently changed, deleted, or otherwise ambiguous copies require reconciliation; modification times never decide which version wins. Read ordinary lists after each stage and compare the expected contents. `storage status` also reports changed, missing, or unreadable retained sources without importing them.

Use `--resolution-file` only for a reviewed exception that the automatic evidence rules refuse. The JSON file is strict and bounded. Every resolution lists all source locators being combined, every inventoried physical path and source digest, exactly one `select` decision, and an explicit `reject` decision for every other source. A target may keep one locator, select an absent-primary source, or collapse several lifecycle locators into one record. Apply re-reads the file and every source; the resolution-file digest is part of the preview digest. Every resolved source locator still undergoes missing-copy and deletion checks across linked worktrees. A resolution cannot conceal a dirty, staged, or committed deletion. Unlisted divergence, changed paths or digests, branch-only variant bytes without an inventoried physical copy and explicit resolution, branch deletion, invalid native output, and invented selected content still refuse cutover. A branch change whose bytes exactly equal the primary physical source is not a distinct variant and does not need a redundant resolution.
...
For a historical collision between unrelated task records, keep one resolution at its existing ID and give the other a new `target_locator` plus `"repair_task_id": "LED-675"`. This repair changes exactly one plain top-level `id:` scalar in the selected retained bytes, requires the target filename to begin with that ID, and rejects any other projection change. If the selected historical bytes also contain the known fused closing-fence defect, migration applies the identity repair first and the projection-preserving newline repair second. The storage capture boundary independently repeats that exact composition from the digest-selected original. Preview and the private recovery manifest report the selected source digest, final target digest, rejected sources, both repairs, and complete original bytes. The source Markdown files are never rewritten.
