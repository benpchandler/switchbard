...
- Live rendered journeys passed against Switchbard (one open PR) and budget (zero open PRs), covering all four pages, filtering to no matches, theme hot reload, and a real invalid-repo refresh retaining stale data. Terminal sizes: 40x8, 80x24, 120x40, 180x50. Captures are named `sbt-navigation-badges-{0,1}.txt` in the OS temporary directory.
