...
- **Live work marker and the claim loop (owner-directed 2026-09-04; TASK-150).**
  ...
  - *`sbt` renders it, never stores it*: the `work` column shows one `●` per
    live session, the whole row wears the `working` band for `work.flash_ms` of every
    `work.period_ms` (lighthouse cadence, owner-tuned 2026-09-04; 0 = steady), the title bar counts `working:N`, and the
