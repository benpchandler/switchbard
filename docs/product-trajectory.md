...
- **Separate planning, priority order and checklist progress (owner-approved 2026-09-15).** Implement Considering / Planned as its own field, a prioritized Planned list, and criterion-count progress across descendants with explicit manual Done and a 100%-needs-review signal. Migrate all tracked task repositories conservatively through core commands with preview, backup and stale guards. Preserve criteria and historical completion. Contract and evidence: `docs/task-planning-progress.md`.
