...
**Direct column positioning (owner-directed experiment loop, 2026-09-18, TASK-212).** E007 previews moving one selected column left/right, with Enter acceptance and Esc undo. It preserves other view settings and the legacy global column-order flow.
...
- **One live TUI with owner-paced experiments (owner-directed 2026-09-17; TASK-286).** Build and commit independently of owner review. The running TUI offers an explicitly requested update when its installed executable changes; the existing installer retains its lineage guard. A compiled experiment catalog provides independent enable/disable and Keep/Remove decisions. Try pins a three-line island across pages; Feedback expands with autosaved drafts and local submissions tied to experiment number and build, outside the task board. Hiding instructions does not disable a feature. Keep enables immediately; code cleanup follows separately. Remove disables and records intent without pretending to delete code. Routine backend fixes preserving intended behavior do not require owner acceptance. The exploration loop uses compilation and bounded manual observations without automated test authoring or execution. First experiment: TASK-251, `t e` opens task editing. Scope, state matrix and evidence: [tui-experiments.md](tui-experiments.md).
...
- **Repository idea and bug capture (owner-directed, TASK-275).** On every TUI page, `i` and `b` open drafts scoped to the repository being viewed; Enter performs the single asynchronous native save and Esc cancels without writing. Repository captures pin their scope and destination through queued saves, failures and retries, use Not started status, Considering (unplanned), low priority and no assignee, assign ideas to `Ideas`, and assign bugs to `Bugs` even without separate project definitions. The legacy `:bug`/`:idea` tool-report route and `report_repo` redirect remain unchanged; per-action destination and metadata configuration is deferred to TASK-276/TASK-277. Current contract and evidence: [repository idea and bug capture](repo-report-capture.md).

## Planned

- **Stable task numbers, hierarchy as a field (owner-directed 2026-09-17, proposed).** A task number exists so a person can name a task to an agent from memory, in text nothing can rewrite later: commits, branches, PR titles, chat. Today a sub-issue's id encodes its position (`TASK-12.1`), so reparenting renames the task and every unreachable reference goes stale. Direction: one flat sequential number per repository for every task including sub-issues, allocated once and never changed (a one-way door); `parent_task_id` always written, with decimal parsing kept as a read-only fallback so existing ids keep resolving; reparent becomes a field update; lineage shown by indentation and a parent chip computed from the parent field rather than encoded in the name. Live renumbering is rejected - the ephemeral per-view row number covers quick reference. A separate user-visible UUID is rejected for now; cross-repository exchange already composes a repo-identity UUID with the task id. Existing decimal ids keep their names forever and nothing migrates. Not implemented or accepted: `docs/task-numbering.md`.

- **Persistent asynchronous decisions (owner-directed 2026-09-16, TASK-113).** The native companion Inbox retains agent questions, context, options, user responses and history in Switchbard's database, with a full `sb decision` protocol and opt-in runtime hooks. Every question automatically offers both “I don’t care - you decide” (scoped delegation) and “I don’t understand this - explain it” (clarification without approval). Silence never answers a question. Agents continue independent work while the affected step stays held; responses remain available until explicit delivery acknowledgement. Integrate the in-progress TASK-143 bug handoffs into one Inbox and preserve xplan's sole mission-writer authority. Feature contract, repository grounding, implementation sequence and future state/stress evidence: `docs/asynchronous-decisions.md`. This is planned, not implemented or visually accepted.
...
- **Emphasis roles: rule-based terminal formatting (owner-directed 2026-09-12, project Emphasis Roles, TASK-175/176/177, TASK-200).** `sbt` paint rules bind facts to theme-defined emphasis roles (quiet, palette slot, strong, alert, band, struck) instead of raw hex, so a rule can sit on any rung of the quiet-to-loud attention ladder and every preset restyles every rule. Rules keep the existing hierarchy; non-conflicting attributes merge, conflicts go to the most specific rule, a trailing `!` stops evaluation, and each rule composes one fill with the ink over it (`theme.highlights` slots `h1`..`h9` beside `band`, TASK-237), so a cell wears one fill, the most specific rule's, patched over only by `selected` and `working`. Targets grow to group headings, the header row and the title line, driven from the cursor. Palette slots, composable roles, stop-if-true, structural targets and cursor-context paint controls are implemented in the Emphasis Roles branch. Active navigation, context counts/filter, bold headers, sort underline and factual priority/completion emphasis establish the default hierarchy. Colored presets paint their declared canvas, including the light preset; rendered APCA tests cover normal, selected, working, detail and navigation surfaces. Speculative and deferred: continuous `scale:` rules. Current behavior and verification: `docs/emphasis-roles/guide.md` and `docs/emphasis-roles/implementation.md`. Research basis, targets and model: `docs/tui-formatting-legibility.md`.
...
- **Task Queue GitHub authority and identity (owner-approved 2026-09-01; TASK-80.2).** The governing contract is `docs/decisions/task-queue-authority-model/`. Native tasks, configured Project sources, Project memberships, GitHub artifacts, generic local-to-GitHub links, and queue projections keep separate identities. Switchbard owns local planning, source selection, source-band order, dispatch custody, and outcome acceptance; GitHub owns Project membership and delivery observations. Queue order is deterministic source bands: the existing local dispatch stack first, configured Projects in binding order, and Project items in GitHub order. A remote-only item becomes individually rankable only through explicit local adoption, which creates a native task and suppresses the standalone projection. Existing URL-string `references:` remain generic rather than pretending to persist typed link kinds. The adapter is read-only, preserves Fresh/Stale/Unavailable and per-field Unknown semantics, and classifies missing-looking responses as `MissingOrInaccessible`, never deletion. Dispatch success becomes one atomic, idempotent native mutation that records label, `In Review`, note, and PR reference together. Delivery never marks a task Done; human outcome acceptance remains local. Deferred: remote-only per-item rank, cross-repo interleaving, GitHub write-back, webhooks, other forges, and typed links.
