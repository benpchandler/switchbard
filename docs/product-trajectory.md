...
- **Repository idea and bug capture (owner-directed, TASK-275).** On every TUI page, `i` and `b` open drafts scoped to the repository being viewed; Enter performs the single asynchronous native save and Esc cancels without writing. Repository captures pin their scope and destination through queued saves, failures and retries, use the native initial status, medium priority and no assignee, leave ideas unassigned, and assign bugs to an existing `Bugs` project when available. The legacy `:bug`/`:idea` tool-report route and `report_repo` redirect remain unchanged; per-action destination and metadata configuration is deferred to TASK-276/TASK-277. Current contract and evidence: [repository idea and bug capture](repo-report-capture.md).

## Planned

- **Stable task numbers, hierarchy as a field (owner-directed 2026-09-17, proposed).** A task number exists so a person can name a task to an agent from memory, in text nothing can rewrite later: commits, branches, PR titles, chat. Today a sub-issue's id encodes its position (`TASK-12.1`), so reparenting renames the task and every unreachable reference goes stale. Direction: identity is a bare sequential number per repository covering every task including sub-issues, allocated once and never changed; `88.312` is a *display* name computed from the parent field and never persisted, so a reparent changes what is shown and nothing that is stored. Resolution treats the suffix as authoritative and the prefix as a hint, correcting a stale one rather than failing. Reparent becomes a field update. Cross-repository views prefix the repository abbreviation the task prefix already provides. One-time renumber folds this repository's nine decimal ids into the sequence by creation date, with permanent append-only aliases so existing references keep resolving. Live renumbering on any ongoing rule is rejected - an ephemeral per-view row number covers quick reference - as is a second user-visible identifier. Not implemented or accepted: `docs/task-numbering.md`.
