...
- **Custom project fields (owner-directed 2026-09-18, TASK-320).** Repository-declared text, enum, date and person attributes now extend to projects, with separate task/project namespaces and native validation and persistence. `sb project field` manages declarations; `sb project create/edit --set` and `--unset` manage values. In sbt, `t f` edits shared project values and `project.<name>` works with columns, filters, sorting, grouping and saved views. Project lifecycle remains separate from domain attributes such as lender stage. No Cambridge-specific field is built in. Local implementation; installation and owner acceptance are separate. Contract and state matrix: [custom project fields](project-custom-fields.md).
