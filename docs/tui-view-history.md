...
Open **Views > History** with `v h`. Each visible entry is a compact card with its own plain-language title, relative time and miniature table. Cards apply their saved presentation settings to current cached data; they are not historical row snapshots. Search by typing, use arrows to select cards, and use PgUp/PgDn to page through cards before pressing Enter to restore. Esc cancels without changing the active view. After restoring, use `v s <number>` to keep it in a numbered slot. `v s d` deliberately changes the repo default; `v g d` promotes the default globally. Neither launch nor automatic capture writes a saved slot.

Committed view changes and page switches update resume immediately, so another session in the same repo can open that arrangement without waiting for the previous session to exit. Filter drafts and cancelled edits are not saved; selection movement alone does not write a checkpoint.
