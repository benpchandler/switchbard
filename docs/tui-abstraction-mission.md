...
The branch integrates preserved Inbox pipeline ancestor `928f162c38dba94499cf2ba8ef756ac91655118d`, including parent-picker ancestry. That intermediate head was explicitly not releasable. Its Inbox-owned named-resume/legacy decoder repair subsequently arrived in descendant `30f188eef217ae0a8446539a3cd31f0a76a7c626`, now merged into this branch without conflict. The abstraction integration preserves that implementation unchanged.

Integration retains four-page navigation and the all-open-PR count. The shared Action catalog now distinguishes task-only, list-only and global actions, so remapped list actions remain blocked on Agents and Inbox; Inbox help reads the same catalog. `ListSettings::for_page` returns no scope for Agents or Inbox, preventing list persistence paths from being invented for them. Parent-picker help follows the configured task prefix. Existing shared column/filter/sort/paint/presentation/view contracts remain in place.

Targeted isolated verification passed: 50 tests across date rollover, shortcut remapping, bounded list presentation, navigation badges, parent picker, view scope, date painting, filtering, PR controls and sorting; eight opt-in live checks were not run in this integration pass. Both inactive Tasks and cached PR date projections refresh when UTC day changes while Inbox is active. TUI test compilation and all-target clippy passed. No full workspace/GUI preflight, integrated live installation, final Inbox acceptance, pipeline, PR or merge is claimed. Next: merge final repaired Inbox descendant, verify named restart and legacy decoding, then complete serialized delivery gates and the owed PR handoff.

### Verification after repaired Inbox integration

Full TUI tests on the integrated source passed: 168 passed, 21 existing opt-in tests ignored, zero failures. Focused core PR tests passed 21/21, and parent relationship tests passed 7/7. Core and TUI all-target clippy passed with warnings denied. Evidence logs are under `/tmp/switchbard-abstractions-evidence/integrated-{tui,core-pr,core-parent,clippy}.log`. The added real `pages3=inbox` fixture restores both independent views and re-encodes exactly one parseable named JSON record, without the obsolete trailing tuple. The earlier pending resume defect is resolved by the imported Inbox repair; it is not an outstanding integration limitation.

The abstraction branch is ready for the delivery pipeline after the other active full preflight completes. PR #144 remains the intended stack base; neither that PR nor the abstraction branch may be merged on this task's authority. Full integrated workspace/preflight, CI, fresh live evidence, and the concrete abstraction PR handoff remain unclaimed. All seven tracker tasks remain In Progress with ball:agent; TASK-144's human confirmation remains open. Preserve the untracked Lavish proposal and the default installed application. The prepared complete delivery intent is `/tmp/switchbard-abstractions-evidence/delivery-intent.txt`.
