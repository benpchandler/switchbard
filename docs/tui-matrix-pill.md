...
The Progress column keeps its `%` header and its place beside Status. Its default appearance is a compact blue capsule that fills from left to right and turns red at exactly 100% using the existing retained-criterion rollup. The same column identity updates existing layouts that already show Progress; no view or task-data migration is needed.
...
The terminal appearance uses rounded Powerline glyphs supported by the owner's configured Nerd Font. A plain-text fallback is available for other fonts. The colored themes use a smooth, solid empty surface with matching end caps rather than a stippled overlay. Fill, completion and shell colors belong to the existing Lua theme system; selected and working-row backgrounds remain visible around the capsule.

## State and stress evidence

| State or invariant | Verification |
| --- | --- |
| No criteria, zero, tiny positive, 12.5%, half, high, near-full, full | Real task fixture, production rendering and endpoint assertions; only exactly full changes from blue to red |
...
| Selected, unselected, working and painted rows; dense adjacent rows | Styled buffer evidence; row background preserved and solid empty body matches the end caps |
...
Set `progress_style = "pill"` (default), `"ascii"`, or `"icons"` in `~/.switchbard/tui.lua`. Existing explicit `glyphs.progress` overrides retain icons unless `progress_style` is set explicitly. Theme surfaces `progress_fill`, `progress_complete`, `progress_empty`, and `progress_shell` control the capsule colors. A custom theme without `progress_complete` falls back to its existing `progress_fill`; an explicitly empty completion style remains respected. `progress_shell` supplies the same solid ink to empty body cells and end caps; `progress_empty` remains the ASCII/plain fallback ink. Plain themes retain a texture distinction when no shell color is defined. At less than six available cells, the renderer uses a compact icon, or an ASCII marker in ASCII mode, instead of clipping a capsule.
