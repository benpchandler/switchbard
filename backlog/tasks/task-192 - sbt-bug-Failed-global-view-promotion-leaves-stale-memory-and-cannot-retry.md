---
id: TASK-192
title: 'sbt bug: Failed global view promotion leaves stale memory and cannot retry'
status: Done
assignee: []
...
Impact: Failed global view promotion leaves stale memory and cannot retry
...
└──────────────────────────────────────────────────────────────────────────────┘
:bug Failed global view promotion leaves stale memory and cannot retry▏
```
...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 A failed second promotion write reports which destination changed and retains the repo override.
- [x] #2 After repairing the repo destination, retry succeeds without losing either view or reporting an artificial external-edit conflict.
<!-- AC:END -->

## Implementation Notes

<!-- SECTION:NOTES:BEGIN -->
Impact: users whose global promotion writes successfully but whose repo override write fails receive stale in-memory state and cannot retry after repairing the file. Medium priority because local settings recovery is blocked, while task data is untouched. Evidence: source review of ViewStore::promote and promote_checked two-file writes during TASK-166 integration; disk-backed E2E simulates second-write failure. Filing screen is report context. Fix must describe partial success honestly and preserve a retry path; no cross-file atomicity claim.
<!-- SECTION:NOTES:END -->
