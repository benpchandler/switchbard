...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Every picker row renders its key (number or letter) and every letter command (r f c o d m g h) is visible inside the picker box, not only the footer
- [ ] #2 apply_picked_value dispatches on a typed PickOption payload; no starts_with / string parsing of option labels remains in app.rs
- [ ] #3 cargo test -p switchbard-tui green with no test asserting on option label prefixes
<!-- AC:END -->
