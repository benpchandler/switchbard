...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Separate parsing and predicate evaluation from entity-specific field access using the shared column contract.
- [x] #2 Tasks and an existing non-task list use the same applicable matching engine through explicit adapters, with unsupported fields handled predictably.
- [x] #3 Tests cover text plus categorical composition, negation where supported, multi-valued fields, missing values, invalid expressions, clear/reset and zero matches.
- [x] #4 Search, filter pickers and paint predicates retain equivalent matching semantics; existing task queries and page isolation remain compatible.
<!-- AC:END -->

## Implementation Notes

<!-- SECTION:NOTES:BEGIN -->
Implemented in feat/tui-abstractions: entity-independent Filter parser/evaluator used by Tasks and PR adapters; legacy task exports retained. E2E verifies text/category composition, negation, multivalued fields, missing/unsupported fields, reset and zero matches; live PR checks passed. Root integration still in progress.
