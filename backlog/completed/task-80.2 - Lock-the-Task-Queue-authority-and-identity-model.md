...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The model distinguishes a local task from a linked or GitHub-backed delivery record without conflating their identities.
- [x] #2 Every mutable field has exactly one authority and every derived field identifies its provenance and observation time.
- [x] #3 Invalid, stale, transferred, deleted, inaccessible, and partially observed GitHub states have explicit behavior.
- [x] #4 Existing repo-local task storage remains valid and migration-free.
<!-- AC:END -->
...
Canonical contract package: docs/decisions/task-queue-authority-model/. It replaces docs/task-queue-authority-model.md. The independent merged fidelity audit accepted the plan, acceptance contract, and verifier with zero blockers and zero minors. The implementation verifier is intentionally RED before production work: 16 FAIL, 1 BLOCKED, 0 skipped; the blocked visual criterion awaits the mixed-source body canonical owned by TASK-80.4.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Locked the Task Queue authority and identity model as an independently audited executable contract. The package defines stable identities, one authority per field, explicit observation/failure states, migration-free compatibility, downstream implementation slices, and a fail-closed 17-criterion verifier. Production implementation remains pending under TASK-80.3 and TASK-80.4.
